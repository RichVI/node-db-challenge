...
- [x] Explain the difference between `Relational Databases` and `SQL`.
      A.Relational databases are databases where the resources can share reference ID's to resources from other tables within the database. SQL is a language for working with databases.

- [x] Why do tables need a `primary key`?
      A. A primary key provides an unique identifier for each row in a table to avoid duplicate data.

- [x] What is the name given to a table column that references the primary key on another table.
      A. Foreign key

- [x] What do we need in order to have a _many to many_ relationship between two tables.
      A. A new table is needed to create the relationship between the two tables.
